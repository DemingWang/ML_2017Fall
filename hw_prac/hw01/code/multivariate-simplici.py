...
def train(X1, t):
    ...
    .. note::
       ...
       Here the equations
       :math:`w = (X^T X)^{-1} (X^T t)` are called *normal equations.*

       The quantity X is called design matrix. X is a matrix of dimension
       :math:`M+1 * N` so it may not be square matrix.

       The quantity :math:`X^{\\textdagger} = (X^T X)^{-1} X^T` is called Moore-Penrose
       pseudo-inverse of matrix X.

       We might also remember matrix inverse property
       :math:`(AB)^-{1} = B^{-1} A^{-1}`
...
def compute_rmse(X, t, w):
    ...
    # Method 1
    # Mean sums all the elements and divide by total number of elements.
