...
def train(X, t):
    ...
    """
    # Solving Normal Equation using matrix inverse property
    # Normal equation is computationally inefficent, and good for small data only.

    # Note: (X^T X)^-1 X    is called Moore-Penrose pseudo-inverse matrix of X.
    w = inv(X.T @ X)  @ X.T @ t   # M = 9: 0.161309. rmse w/o regu

    # XXX  WARNING:
    # w = (inv(X.T @ X))  @ (X.T @ t)  # M = 9: 0.161190. rmse w/o regu
    # w = (inv(X.T @ X))  @ X.T @ t    # M = 9: 0.161309. rmse w/o regu
    # Xpseudo = ((inv(X.T @ X))  @ X.T) = (inv(X.T @ X))  @ X.T @ t = inv(X.T @ X)  @ X.T @ t


    # Matrix associative property ABC = A (BC) = (AB)C
    # fails here, above 2 gives two different final answers here.
    # A (BC)  M = 9: 0.161190.
    # A B C  M = 9: 0.161309.
...
def comparison(fh_train,fh_test,fh_valid, lam_min_rmse_valid,M):
    ...
    print('Test RMSE without regularization for M = 9: %0.6f.' % E_rms_test)

    w_ridge = train_regularized(Xtrain,ttrain, float(lam_min_rmse_valid), M)
    E_rms_test = compute_rmse(Xtest, ttest, w_ridge)
    print('Test RMSE with    regularization for M = 9: %0.6f.' % E_rms_test)
