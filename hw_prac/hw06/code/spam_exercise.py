...
def main():
    ...
    idir       = '../data/spam/'
    odir       = 'outputs/'
    fdata_tr   = idir + 'spam_train.txt'
    fdata_ts   = idir + 'spam_test.txt'

    # sparse data
    fsparse_tr = odir + 'spam_train_svm.txt'
    fsparse_ts = odir + 'spam_test_svm.txt'

    # feature matrix (vocab) ONLY FROM TRAIN!!!
    fvocab     = odir + "spam_vocab.txt"

    # dense data (input for perceptron)
    fdense_tr  = odir + 'spam_train_dense.txt'
    fdense_ts  = odir + 'spam_test_dense.txt'
